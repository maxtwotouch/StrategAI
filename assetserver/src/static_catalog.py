...
class StaticCatalog:
    ...
    Unit family (flat naming — archer.png, scout.png, etc.):
        _unit_catalog["archer"] = [full_path]
        _unit_catalog["scout"]  = [full_path]
    """
    ...
    def __init__(self, root_dir: str | None = None):
        ...
        self._unit_catalog: dict[str, list[str]] = defaultdict(list)
        self._scan()
    ...
    def list_unit_types(self) -> list[str]:
        """Return unit types that have at least one PNG."""
        return sorted(k for k, v in self._unit_catalog.items() if v)

    def has_unit_type(self, unit_type: str) -> bool:
        """Return True if this unit type has at least one PNG."""
        return bool(self._unit_catalog.get(unit_type))
    ...
    def _scan_unit(self, path: str) -> None:
        """Flat folder: ``archer.png``, ``scout.png``, ``warrior.png``, etc.

        The filename stem (without extension) becomes the unit type key.
        """
        for fname in sorted(os.listdir(path)):
            if not fname.lower().endswith(".png"):
                continue
            unit_type = os.path.splitext(fname)[0]  # e.g. "archer"
            full = os.path.join(path, fname)
            self._unit_catalog[unit_type].append(full)
