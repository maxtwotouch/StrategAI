...
Project for dataset generation, curation, and processing the dataset into export format.
...
- `src/prepare_hf_dataset.py`
   - Converts Version 1 data into Version 2 (`v2`).
- `src/validate_hf_export.py`
   - Validates `v2` exports before publishing (CI-friendly reports and exit codes).
- `src/remove_bad_assets.py`
   - Optional helper to remove assets by ID and clean manifests.
- `config/structure_workflow.json`
   - Primary ComfyUI workflow for non-tile assets.
- `config/background_tile_workflow.json`
...
- `dataset/v1/images/*.png`
- `dataset/v1/metadata/*.json`
- `dataset/v1/dataset_manifest.jsonl`
- `dataset/v1/run_report.json`

### 2) Curate quality (delete bad images)
Delete bad PNGs from `dataset/v1/images/` by visual review. `prepare_hf_dataset.py` will only include assets whose PNGs still exist in the next step, so this is the main curation mechanism.

### 3) Export Version 2 (`v2`)
...
- `dataset/v2/images/*.png`
- `dataset/v2/images/*.txt`
- `dataset/v2/metadata.jsonl`
- `dataset/v2/prepare_hf_report.json`
- optional: `dataset/v2/metadata/*.json`

### 4) Validate export before publish

```bash
python3 src/validate_hf_export.py ./dataset/v2
```

By default, this writes:

- `dataset/v2/validation_report.json`
- `dataset/v2/validation_issues.jsonl`

Useful strict modes:

```bash
python3 src/validate_hf_export.py ./dataset/v2 --require-caption-txt
python3 src/validate_hf_export.py ./dataset/v2 --require-caption-txt --fail-on-warnings
python3 src/validate_hf_export.py ./dataset/v2 --skip-orphan-check
```

## Metadata Contracts

### Version 1 sidecar schema (`dataset/v1/metadata/*.json`)
...
### Version 2 HF metadata schema (`dataset/v2/metadata.jsonl`)
...
- Curate by deleting PNGs only, then regenerate `v2` from remaining assets.
...
```bash
python3 src/prepare_hf_dataset.py ./dataset --out-dir ./dataset/v2 --fail-on-missing-image
python3 src/prepare_hf_dataset.py ./dataset --out-dir ./dataset/v2 --fail-on-duplicate-id
python3 src/prepare_hf_dataset.py ./dataset --out-dir ./dataset/v2 --skip-captions-txt
python3 src/prepare_hf_dataset.py ./dataset --out-dir ./dataset/v2 --keep-source-ids
```
