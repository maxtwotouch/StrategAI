...
| 🖌️ **Style Adaptation** | LoRA fine-tuning via knowledge distillation | 100-image dataset generated by FLUX.2 [dev] (32B) teacher → FLUX.2 Klein 4B student LoRA, 6-experiment matrix |
...
> **Why no top-level `requirements.txt`?** Each subproject manages its own dependencies independently — the backend uses `pyproject.toml` (`pip install -e .`), the asset server uses `pyproject.toml` or `requirements.txt`, and the frontend uses `package.json`. A single top-level requirements file would conflate unrelated dependency trees and conflicting Python version constraints (3.10+ vs 3.11+). Similarly, there is no `Dockerfile` or Docker Compose setup — our deployment environment does not have root access or Docker available. The system runs as plain OS processes (see [`DEPLOYMENT.md`](DEPLOYMENT.md)).
...
- **Knowledge distillation pipeline**: A larger [FLUX.2 [dev]](https://huggingface.co/black-forest-labs/FLUX.2-dev) (32B) DiT teacher model with [Multi-Angles LoRA v2](https://huggingface.co/lovis93/Flux-2-Multi-Angles-LoRA-v2) generates the training dataset, distilling top-down perspective knowledge into a compact 100-image set used to train the FLUX.2 Klein 4B student LoRA
...
| Document | Contents |
|----------|----------|
| [`docs/ARCHITECTURE.md`](docs/ARCHITECTURE.md) | System architecture, engine layers, LLM integration design (includes separation-of-concerns rationale) |
...
| Document | Contents |
|----------|----------|
| [`docs/report/REPORT.pdf`](docs/report/REPORT.pdf) | Final report |
...
> 📖 See [`DEPLOYMENT.md`](DEPLOYMENT.md) for the deployment guide — the system runs as plain processes (no Docker/containers; see rationale there).
...
2. **Pick a task**: See the [open issues](https://github.com) or the subproject READMEs for outstanding work
...
Background music tracks bundled in `frontend/public/audio/` are sourced from
**OpenGameArt.org** under Creative Commons licenses. Full attribution —
including titles, authors, source URLs, and license terms — is maintained in
[`frontend/public/audio/ATTRIBUTION.md`](frontend/public/audio/ATTRIBUTION.md).

| Track | Author | License |
|-------|--------|---------|
| ambient.mp3 ("The Traveller") | Viktor Kraus | [CC-BY 3.0](https://creativecommons.org/licenses/by/3.0/) |
| bards-tale.mp3 ("Medieval: The Bard's Tale") | RandomMind | [CC0](https://creativecommons.org/publicdomain/zero/1.0/) |
| medieval-battle.mp3 ("Medieval: Battle") | RandomMind | [CC0](https://creativecommons.org/publicdomain/zero/1.0/) |
