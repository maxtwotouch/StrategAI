# PixelArtDiffusionCheckpoint Dataset Pipeline
...
- `src/prompt_generator.py`
  - Builds prompt-pack JSONL input used by the generator.
- `src/extract_prompt_templates.py`
  - Extracts reusable prompt templates from an existing prompt-pack.
...
- Prompt pack JSONL (for example from `src/prompt_generator.py`).
- Workflow API JSON files in `config/`.

## End-to-End Flow

### 0) (Optional) Generate prompt pack
...
- Use fixed prompt-pack inputs and pinned workflow JSON files in `config/`.
