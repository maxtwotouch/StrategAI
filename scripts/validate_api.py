...
Examples:
  ...
  python scripts/validate_api.py --base-url http://192.168.1.50:8000 --family tile
    ...
    p.add_argument("--payloads-file", type=Path, default=None,
                   help="Replay payloads from a JSON file (generated by scripts/generate_payloads.py)")
    return p.parse_args()
...
async def run_payloads_file(base_url: str, client: httpx.AsyncClient,
                           payloads_file: Path, timeout: float,
                           verbose: bool) -> list[dict]:
    """Replay payloads from a JSON file and validate responses.

    Auto-detects the endpoint for each payload based on its field shape.
    Supports both JSON arrays and JSONL (one object per line) formats.
    """
    import json as _json

    results: list[dict] = []
    print("\n" + "=" * 64)
    print(f"  PAYLOADS FILE REPLAY — {payloads_file}")
    print("=" * 64)

    # Load payloads
    raw = payloads_file.read_text(encoding="utf-8")
    payloads: list[dict] = []
    ...
    if not payloads:
        print("    ✗ No valid payloads found in file")
        return results
    ...
    for i, payload in enumerate(payloads):
        ...
        endpoint = _detect_endpoint(clean)
        if endpoint is None:
            print(f"    ⚠ #{i+1:4d}  Unknown payload shape — skipping")
            failures += 1
            continue

        # Skip action payloads with placeholder leader_id
        if endpoint == "/leader" and clean.get("asset_type") == "action" \
                and clean.get("leader_id", "").startswith("REPLACE_"):
            if verbose:
                print(f"    ⚠ #{i+1:4d}  Skipping action payload with placeholder leader_id")
            continue

        r = await _post(client, f"{base_url}{endpoint}", clean, timeout)
        status_icon = _icon(r)

        # Validate response
        if r.status_code == 200 and isinstance(r.response_body, dict):
            details: list[str] = []
            asset_type = clean.get("asset_type", endpoint.lstrip("/"))
            if endpoint == "/structure" or endpoint == "/object" or endpoint == "/terrain":
                family = endpoint.lstrip("/")
                r.checks_passed = validate_tile_response(r.response_body, family, details)
            elif endpoint == "/unit":
                r.checks_passed = validate_unit_response(r.response_body, details)
            elif endpoint == "/leader":
                r.checks_passed = validate_leader_response(r.response_body, asset_type, details)
            elif endpoint == "/background_tile":
                r.checks_passed = validate_background_tile_response(r.response_body, details)
            r.checks_failed = len(details) - r.checks_passed
            r.check_details = details
        ...
        if verbose:
            print(f"    {status_icon} #{i+1:4d} [{r.status_code}] {r.elapsed_ms:6.0f}ms  {endpoint}")
            for d in r.check_details:
                print(f"      {d}")
        elif i % max(1, len(payloads) // 10) == 0 or r.status_code != 200:
            print(f"    {status_icon} #{i+1:4d} [{r.status_code}] {r.elapsed_ms:6.0f}ms  {endpoint}")

        results.append(_result_dict(r, "bulk"))

        if consecutive_fails >= CONSECUTIVE_FAILURE_THRESHOLD:
            print(f"    ⚠ {consecutive_fails} consecutive failures — stopping replay")
            break
...
def _detect_endpoint(payload: dict) -> str | None:
    """Auto-detect the API endpoint from payload field shape."""
    keys = set(payload.keys())

    if keys >= {"tile_type"} and "category" not in keys and "unit_type" not in keys:
        return "/background_tile"
    if keys >= {"unit_type"}:
        return "/unit"
    if keys >= {"asset_type"}:
        return "/leader"
    if keys >= {"category", "style", "condition", "scale"}:
        return "/structure"
    if keys >= {"category", "biome", "season"}:
        return "/object"
    if keys >= {"category", "scale", "material"}:
        return "/terrain"
    return None
...
async def main() -> None:
    ...
    # ------------------------------------------------------------------
    # Payloads-file replay mode — skip normal suites, just replay payloads
    # ------------------------------------------------------------------
    if args.payloads_file:
        if not args.payloads_file.exists():
            print(f"✗ Payloads file not found: {args.payloads_file}", file=sys.stderr)
            sys.exit(1)

        print(f"🔍 API Validation Script — Payloads Replay Mode")
        print(f"   Base URL: {args.base_url}")
        print(f"   Payloads file: {args.payloads_file}")
        print(f"   Timeout: {args.timeout}s")

        report = RunReport(base_url=args.base_url)
        async with httpx.AsyncClient(timeout=httpx.Timeout(args.timeout)) as client:
            report.bulk_results = await run_payloads_file(
                args.base_url, client, args.payloads_file,
                args.timeout, args.verbose)

        # Compute totals
        passed = sum(1 for r in report.bulk_results if r.get("checks_failed", 1) == 0)
        failed = len(report.bulk_results) - passed
        report.totals["bulk"] = {"passed": passed, "failed": failed}
        ...
        print(f"  {'bulk':10s}:  ✓ {passed:4d} passed  ✗ {failed:4d} failed  ({pct:.0f}%)")
        ...
        report_dict = {
            ...
            "args": {
                "payloads_file": str(args.payloads_file),
        ...
        if failed > 0:
            print(f"⚠ {failed} failures detected — check the report for details.")
            sys.exit(1)
        else:
    ...
    # ------------------------------------------------------------------
    # Normal mode — smoke tests, edge cases, leader workflow, bulk gen
    # ------------------------------------------------------------------
