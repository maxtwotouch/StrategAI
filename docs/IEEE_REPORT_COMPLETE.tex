...
Traditional game AI relies on hand-crafted decision systems. Behavior trees provide hierarchical task decomposition but require extensive manual authoring for each new behavior. Finite state machines offer predictable transitions between discrete states but become unwieldy as the number of states grows. Utility-based systems enable more nuanced decision-making through weighted scoring functions, but they still require designers to enumerate and score every possible action.

Recent work has begun exploring LLMs as a more flexible foundation for game AI. Park et al.\ demonstrated ``generative agents'' that simulate human behavior through natural language reasoning in a sandbox environment, producing remarkably believable social interactions. However, their system operated without competitive gameplay or rule enforcement, leaving open the question of how LLM-driven agents perform in adversarial, rule-bound environments. Zhu et al.\ used GPT-4 to generate dialogue and interpret commands for Minecraft agents, but the underlying decision-making remained scripted. Volum et al.\ proposed ``CraftAssist,'' also for Minecraft, using LLMs to translate natural language instructions into game actions, though the AI did not autonomously pursue strategic goals. Wang et al.~\cite{wang2023voyager} introduced Voyager, an LLM-powered agent that autonomously explores Minecraft through self-generated skill libraries and iterative code generation, establishing LLMs as viable lifelong learners in open-ended game environments. Schick et al.~\cite{schick2023toolformer} demonstrated that LLMs can teach themselves to invoke external tools via API calls---a capability that directly motivates our intent-based abstraction, where the LLM selects strategic intents through structured tool calls while the deterministic game engine handles precise execution.

StrategAI differs from these works by employing LLMs as fully autonomous strategic agents in a competitive, rule-bound environment where three AI civilizations must make real-time decisions about expansion, warfare, diplomacy, and resource management. Our intent-based abstraction addresses a key challenge that prior work left unresolved: LLMs excel at high-level reasoning but struggle with the precise numerical calculations and spatial logic that game mechanics demand. By separating strategic intent from tactical execution, we leverage LLM strengths while maintaining the deterministic, reproducible game mechanics that strategy games require.

\subsection{Procedural Content Generation}

Procedural Content Generation (PCG) has a long and storied history in games, from the randomly generated dungeons of early roguelikes to the algorithmically generated planets of modern titles like No Man's Sky. Traditional PCG relies on algorithmic techniques that, while powerful, produce content within a fixed aesthetic defined by the algorithm itself. Noise functions generate natural-looking terrain but cannot easily produce specific artistic styles. Grammar-based systems can model architectural styles but require extensive rule authoring. Constraint solvers enable designer-specified requirements but scale poorly to complex, multi-constraint problems.

Deep learning has significantly expanded PCG capabilities in recent years. Generative Adversarial Networks have been applied to game level generation and texture synthesis, but their well-known training instability and mode collapse problems limit practical adoption in production pipelines. Variational Autoencoders offer more stable training dynamics but tend to produce blurry outputs that are unsuitable for the crisp, detailed aesthetic that pixel art demands.

Diffusion models have emerged as the current state-of-the-art for image generation, offering both high-quality outputs and diverse variations from a single prompt. Peebles and Xie~\cite{peebles2023scalable} advanced this paradigm by replacing the U-Net backbone with a pure Transformer architecture, creating Diffusion Transformers (DiTs) that exhibit superior scaling behavior and prompt adherence. Liu et al.~\cite{liu2023rectified} proposed rectified flow, an alternative to score-based diffusion that learns straight-line trajectories for substantially faster sampling with fewer inference steps---a technique adopted by the FLUX model family~\cite{esser2024scaling} that powers our asset pipeline. However, their application to game assets---as opposed to concept art or illustrations---remains limited. Game assets have specific technical requirements that concept art does not: transparent backgrounds for compositing, consistent camera perspective across all assets in a family, tileable textures for terrain, and a unified visual style that makes hundreds of independently generated assets look like they belong in the same game.

StrategAI addresses these requirements through a multi-stage pipeline that combines diffusion model inference with deterministic post-processing (background removal via ISNet, Lanczos downscaling, image sharpening) and a structured four-layer prompt system that enforces asset specifications through enum-constrained vocabularies rather than free-form text input.
...
Several community projects have applied LoRA to game-adjacent asset generation. PixelArt-LoRA adapts Stable Diffusion for pixel-art style but focuses on side-view character sprites rather than the top-down perspective required by strategy games. TopDown-LoRA targets overhead camera angles but was trained on general imagery rather than the specific medieval pixel-art aesthetic that StrategAI requires.

Our approach differs from these prior efforts in three significant ways. First, we target FLUX.2 Klein 4B Distilled, a newer Diffusion Transformer architecture~\cite{peebles2023scalable} with substantially improved prompt adherence compared to the U-Net-based Stable Diffusion models used in prior work. FP8 quantization techniques~\cite{lin2024fp8} further reduce the model's VRAM footprint from 16~GB to approximately 8.4~GB, making it viable on consumer-grade GPUs such as the RTX 3070 and 4070. Second, rather than training a single LoRA configuration, we systematically evaluate the interaction between caption design and model capacity through a six-experiment matrix crossing three caption detail levels with two LoRA rank configurations. Third, we integrate the fine-tuned model directly into a production asset generation pipeline with automated prompt assembly, caching, and graceful degradation, demonstrating that LoRA-adapted models can serve real-time game asset requests.

\subsection{Multi-Agent Systems}

Multi-agent systems (MAS) in games typically involve homogeneous agents that share decision logic and differ only in parameter values. This approach is simple to implement but produces agents that feel interchangeable. Heterogeneous multi-agent systems, where agents have genuinely different capabilities, goals, or decision-making strategies, are more complex to build but enable the kind of rich, asymmetric interactions that make strategy games compelling.

StrategAI implements heterogeneous AI agents through persona-based prompting rather than through separate model architectures or training procedures. Each of the three AI civilizations receives a unique character description---Genghis Khan as an aggressive expansionist who remembers every insult, Cleopatra as a diplomatic manipulator who plays factions against each other, Gandhi as a peaceful developer who pursues victory through science and culture---that is appended to a shared base system prompt. This approach enables genuinely diverse strategic behaviors, including emergent phenomena like coalition formation and revenge dynamics, without requiring separate AI implementations for each civilization.
...
The complete StrategAI codebase is available as an open-source repository at \url{https://github.com/strategai/StrategAI}. The curated training dataset of 100 top-down medieval pixel-art images with caption sidecars is published on Hugging Face at \url{https://huggingface.co/datasets/stixxert/topdown-medieval-pixelart}. The trained LoRA adapter weights (detailed\_high, step 1800, 353~MB) are also available on Hugging Face at \url{https://huggingface.co/stixxert/strategai-topdown-medieval-style-lora}. All components are released under the Apache 2.0 license to enable unrestricted use in both academic and commercial contexts.
